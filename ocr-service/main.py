...
async def refresh_cache():
    """
    Manually refresh the invoice cache from MongoDB.
    Useful after deleting invoices or making bulk changes.
    """
    try:
        document_processor.fraud_detector.refresh_cache()
        cache_size = len(document_processor.fraud_detector.invoice_cache)
        logger.info(f"✅ Cache refreshed successfully. Current size: {cache_size} invoices")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Cache refresh error: {str(e)}"
        )
...
async def clear_cache():
    """
    Manually clear the invoice cache (empties it completely).
    Use this when you want to start fresh without database data.
    """
    try:
        old_size = len(document_processor.fraud_detector.invoice_cache)
        document_processor.fraud_detector.invoice_cache = []
        logger.info(f"✅ Cache cleared successfully. Removed {old_size} invoices")
        
        return {
            "success": True,
            "message": f"Cache cleared successfully. Removed {old_size} invoices",
...
async def get_cache_status():
    ...
    Shows how many invoices are currently in memory.
    """
    try:
        cache_size = len(document_processor.fraud_detector.invoice_cache)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Cache status error: {str(e)}"
        )
